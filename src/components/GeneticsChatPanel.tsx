...
export default function GeneticsChatPanel({
  inputSummary,
  analysisSnapshot,
  tier,
  language,
}: Props) {
  const t = useTranslations("Genetics");
  ...
  async function send(textOverride?: string) {
    ...
    try {
      const r = await fetch("/api/genetics-chat", {
        method: "POST",
  ...
  return (
    <div className="rounded-2xl border border-surface-border bg-white dark:bg-slate-900 overflow-hidden shadow-sm">
      <div className="flex items-center justify-between gap-3 px-5 py-4 border-b border-surface-border/70 bg-gradient-to-r from-fuchsia-500/5 via-fuchsia-400/3 to-transparent">
        <div className="flex items-center gap-3 min-w-0">
          <div className="w-9 h-9 rounded-xl bg-gradient-to-br from-fuchsia-500 to-fuchsia-700 flex items-center justify-center flex-shrink-0">
        ...
        {conversationStarted && (
          <button
            onClick={newChat}
            className="text-xs font-semibold text-ink-secondary hover:text-fuchsia-600 px-3 py-1.5 rounded-lg border border-surface-border hover:border-fuchsia-300 transition-colors"
          >
            {t("chatClear")}
          </button>
        )}
            ...
            <div className="grid grid-cols-1 sm:grid-cols-2 gap-2">
              {SUGGESTED_QUESTIONS_KEYS.map((key) => (
                <button
                  key={key}
                  onClick={() => send(t(key))}
                  className="group text-left rounded-xl border border-surface-border bg-white dark:bg-slate-900 hover:border-fuchsia-400/40 hover:bg-fuchsia-50/50 dark:hover:bg-fuchsia-950/20 transition-all px-3.5 py-2.5"
                >
      ...
      <div className="border-t border-surface-border/70 bg-gradient-to-b from-transparent to-surface-raised/30 px-4 py-3">
        <form
          ...
          className={`relative rounded-2xl border bg-white dark:bg-slate-950 transition-all shadow-sm ${
            hasInput || pending
              ? "border-fuchsia-400/50 shadow-fuchsia-500/5"
              : "border-surface-border"
          } focus-within:border-fuchsia-500/70 focus-within:ring-1 focus-within:ring-fuchsia-500/30`}
        >
          <textarea
            ref={textareaRef}
            value={input}
            ...
            {pending ? (
              ...
              <button
                ...
                className={`inline-flex items-center gap-1.5 px-3.5 py-1.5 rounded-lg text-xs font-bold transition-all ${
                  hasInput
                    ? "bg-fuchsia-600 hover:bg-fuchsia-700 text-white shadow-sm"
