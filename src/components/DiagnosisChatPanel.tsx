...
export default function DiagnosisChatPanel({
  ...
  async function send(textOverride?: string) {
    ...
    try {
      const r = await fetch("/api/diagnosed-chat", {
        method: "POST",
            ...
            <div className="grid grid-cols-1 sm:grid-cols-2 gap-2">
              {SUGGESTED_KEYS.map((key) => (
                <button
                  key={key}
                  onClick={() => send(t(key))}
                  className="group text-left rounded-xl border border-surface-border bg-white dark:bg-slate-800 hover:border-brand-blue/40 hover:bg-brand-blue/5 transition-all px-3.5 py-2.5"
                >
      ...
      <div className="border-t border-surface-border/70 bg-gradient-to-b from-transparent to-surface-raised/30 px-4 py-3">
        ...
        >
          <textarea
            ...
            placeholder={t("diagChatPlaceholder")}
            rows={1}
            disabled={pending}
