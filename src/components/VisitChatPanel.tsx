...
export default function VisitChatPanel({
  ...
  async function send(textOverride?: string) {
    ...
    try {
      const r = await fetch("/api/visit-chat", {
        method: "POST",
  ...
  const suggestedKeys = mode === "prep" ? PREP_QUESTION_KEYS : DEBRIEF_QUESTION_KEYS;
        ...
        <div className="flex items-center gap-3 min-w-0">
          ...
          <div className="min-w-0">
            ...
            <p className="text-[11px] text-ink-tertiary leading-tight mt-1">
              {mode === "prep" ? t("chatSubtitlePrep") : t("chatSubtitleDebrief")}
            </p>
          </div>
        </div>
        {conversationStarted && (
        ...
        {!conversationStarted ? (
          <div>
            <p className="text-sm text-ink-secondary leading-relaxed mb-4">
              {mode === "prep" ? t("chatIntroPrep") : t("chatIntroDebrief")}
            </p>
            <div className="grid grid-cols-1 sm:grid-cols-2 gap-2">
              {suggestedKeys.map((key) => (
                <button
                  key={key}
                  onClick={() => send(t(key))}
                  className="group text-left rounded-xl border border-surface-border bg-white dark:bg-slate-900 hover:border-brand-blue/40 hover:bg-brand-blue/5 dark:hover:bg-brand-blue/10 transition-all px-3.5 py-2.5"
                >
