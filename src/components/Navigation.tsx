...
function ToolsDropdown({ pathname }: { pathname: string }) {
  ...
  const TOOLS = [
    ...
    },
    {
      ...
      description: t("womensHealthDesc"),
      color: "text-violet-600 dark:text-violet-400",
      bg: "bg-gradient-to-br from-rose-50 to-violet-50 dark:from-rose-900/20 dark:to-violet-900/20",
      icon: (
        <svg viewBox="0 0 20 20" fill="none" stroke="currentColor" strokeWidth={1.6} className="w-4 h-4">
          <path strokeLinecap="round" strokeLinejoin="round" d="M10 3a3.5 3.5 0 100 7 3.5 3.5 0 000-7zm0 7v6m-2.5-2.5h5" />
        </svg>
      ),
    },
  ];
            ...
            {mobileToolsOpen && (
              <div className="ml-4 space-y-1 pb-1">
                {[
                  ...
                  { href: "/womens-health", label: t("womensHealth"),       color: "text-violet-600 dark:text-violet-400"   },
                ].map((tool) => (
                  <Link
                    key={tool.href}
                    href={tool.href}
