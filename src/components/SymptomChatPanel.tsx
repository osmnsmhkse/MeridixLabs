...
export default function SymptomChatPanel({
  symptom,
  analysisSnapshot,
  language,
}: Props) {
  ...
  async function send(textOverride?: string) {
    ...
    try {
      const r = await fetch("/api/symptom-chat", {
        method: "POST",
  ...
  return (
    <div className="rounded-2xl border border-surface-border bg-white dark:bg-slate-900 overflow-hidden shadow-sm">
      {/* Header */}
      <div className="flex items-center justify-between gap-3 px-5 py-4 border-b border-surface-border/70 bg-gradient-to-r from-violet-500/5 via-violet-400/3 to-transparent">
      ...
      <div className="border-t border-surface-border/70 bg-gradient-to-b from-transparent to-surface-raised/30 px-4 py-3">
        ...
        >
          <textarea
            ...
            placeholder={t("symptomChatPlaceholder")}
            rows={1}
            disabled={pending}
