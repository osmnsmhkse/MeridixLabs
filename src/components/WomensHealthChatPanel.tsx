...
export default function WomensHealthChatPanel({
  mode,
  inputSummary,
  analysisSnapshot,
  tier,
  ...
  const suggestedKeys =
    mode === "a"
      ? (["chatQ1A", "chatQ2A", "chatQ3A", "chatQ4A"] as const)
      : mode === "b"
      ? (["chatQ1B", "chatQ2B", "chatQ3B", "chatQ4B"] as const)
      : (["chatQ1C", "chatQ2C", "chatQ3C", "chatQ4C"] as const);
  ...
  async function send(textOverride?: string) {
    ...
    try {
      const r = await fetch("/api/womens-health-chat", {
        method: "POST",
  ...
  return (
    <div className="rounded-2xl border border-surface-border bg-white dark:bg-slate-900 overflow-hidden shadow-sm">
      {/* Header — rose→violet gradient mark */}
      <div className="flex items-center justify-between gap-3 px-5 py-4 border-b border-surface-border/70 bg-gradient-to-r from-rose-500/5 via-violet-400/5 to-transparent">
        <div className="flex items-center gap-3 min-w-0">
          <div className="w-9 h-9 rounded-xl bg-gradient-to-br from-rose-500 to-violet-600 flex items-center justify-center flex-shrink-0">
            <MeridixMark className="w-4 h-4 text-white" />
          </div>
          <div className="min-w-0">
            <p className="text-sm font-bold text-ink leading-none">{t("chatTitle")}</p>
        ...
        {conversationStarted && (
          <button
            onClick={newChat}
            className="text-xs font-semibold text-ink-secondary hover:text-violet-600 px-3 py-1.5 rounded-lg border border-surface-border hover:border-violet-300 transition-colors"
          >
            {t("chatClear")}
          </button>
        )}
            ...
            <div className="grid grid-cols-1 sm:grid-cols-2 gap-2">
              {suggestedKeys.map((key) => (
                <button
                  key={key}
                  onClick={() => send(t(key))}
                  className="group text-left rounded-xl border border-surface-border bg-white dark:bg-slate-900 hover:border-violet-400/40 hover:bg-violet-50/50 dark:hover:bg-violet-950/20 transition-all px-3.5 py-2.5"
                >
      ...
      <div className="border-t border-surface-border/70 bg-gradient-to-b from-transparent to-surface-raised/30 px-4 py-3">
        <form
          ...
          className={`relative rounded-2xl border bg-white dark:bg-slate-950 transition-all shadow-sm ${
            hasInput || pending
              ? "border-violet-400/50 shadow-violet-500/5"
              : "border-surface-border"
          } focus-within:border-violet-500/70 focus-within:ring-1 focus-within:ring-violet-500/30`}
        >
          <textarea
            ref={textareaRef}
            value={input}
            ...
            {pending ? (
              ...
              <button
                ...
                className={`inline-flex items-center gap-1.5 px-3.5 py-1.5 rounded-lg text-xs font-bold transition-all ${
                  hasInput
                    ? "bg-gradient-to-br from-rose-600 to-violet-600 hover:from-rose-700 hover:to-violet-700 text-white shadow-sm"
