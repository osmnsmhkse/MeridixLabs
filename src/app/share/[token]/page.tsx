...
export default function SharedReportPage({ params }: { params: Promise<{ token: string }> }) {
  ...
  if (loading) return <div className="min-h-screen flex items-center justify-center text-sm text-ink-tertiary">Loading shared report…</div>;
  if (error || !data) {
    return (
      <div className="min-h-screen flex items-center justify-center px-4 bg-surface">
        <div className="max-w-md text-center">
          <h1 className="text-xl font-bold text-ink">Couldn&apos;t open this report</h1>
          <p className="mt-2 text-sm text-ink-secondary">{error ?? "The link may be expired or invalid."}</p>
          <Link href="/" className="mt-4 inline-block text-sm text-brand-blue hover:underline">Go to Meridix Labs</Link>
        </div>
      </div>
    );
  }
    ...
    <div className="min-h-screen bg-white dark:bg-slate-950 py-10 px-4 sm:px-6 lg:px-8 print:py-2 print:px-0">
      <div className="max-w-3xl mx-auto">
        ...
        <div className="hidden print:block mb-6 pb-4 border-b border-surface-border">
          <p className="text-xs text-ink-tertiary">Generated by Meridix Labs · meridixlabs.com</p>
        </div>

        {/* Header */}
        <div className="flex items-start justify-between gap-4 mb-6">
          <div>
            <p className="text-xs font-semibold text-brand-blue uppercase tracking-wider">Shared lab analysis</p>
            <h1 className="mt-1 text-2xl font-bold text-ink">{analysis.source_filename ?? "Lab report"}</h1>
            <p className="mt-1 text-sm text-ink-secondary">
              {formatDate(analysis.report_date ?? analysis.created_at)} · Shared by {sharedBy}
            </p>
          </div>
          <button
        ...
        <div className="rounded-2xl border border-surface-border p-5 mb-5">
          <div className="flex items-start gap-5">
            ...
            <div className="min-w-0">
              <p className="text-xs font-semibold text-ink-tertiary uppercase tracking-wide">Summary</p>
              {analysis.summary ? (
                <p className="mt-1 text-sm text-ink leading-relaxed">{analysis.summary}</p>
              ) : (
                <p className="mt-1 text-sm text-ink-tertiary">No summary available.</p>
              )}
            </div>
          </div>
        </div>
        ...
        {flags.length > 0 && (
          <div className="mb-5">
            <p className="text-xs font-semibold text-ink-tertiary uppercase tracking-wide mb-2">Flagged values</p>
            <div className="rounded-2xl border border-surface-border overflow-hidden">
              <table className="w-full text-sm">
                <thead className="bg-surface-raised">
                  <tr>
                    <th className="text-left px-4 py-2 font-semibold text-ink-tertiary text-xs uppercase tracking-wide">Marker</th>
                    <th className="text-left px-4 py-2 font-semibold text-ink-tertiary text-xs uppercase tracking-wide">Value</th>
                    <th className="text-left px-4 py-2 font-semibold text-ink-tertiary text-xs uppercase tracking-wide">Reference</th>
                    <th className="text-left px-4 py-2 font-semibold text-ink-tertiary text-xs uppercase tracking-wide">Status</th>
                  </tr>
                </thead>
        ...
        {labs.length > 0 && (
          <div className="mb-5">
            <p className="text-xs font-semibold text-ink-tertiary uppercase tracking-wide mb-2">All values ({labs.length})</p>
            <div className="rounded-2xl border border-surface-border overflow-hidden">
              <table className="w-full text-sm">
                <thead className="bg-surface-raised">
                  <tr>
        ...
        <div className="mt-8 pt-5 border-t border-surface-border text-xs text-ink-tertiary">
          <p>This is an AI-generated interpretation — not medical advice. Confirm important findings with a qualified physician.</p>
          {expiresAt && <p className="mt-1">Link expires {formatDate(expiresAt)}.</p>}
          <p className="mt-3">
            Generated by{" "}
